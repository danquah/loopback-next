...
import {InvokeMiddlewareProvider} from '../../../providers';
...
describe('Middleware in sequence', () => {
  ...
  class SequenceWithTwoInvokeMiddleware extends DefaultSequence {
    ...
    protected invokeMiddlewareAfterInvoke: InvokeMiddleware = () => {};

    async handle(context: RequestContext): Promise<void> {
      try {
        const {request, response} = context;
        // The default middleware chain
        if (this.invokeMiddleware != null) {
          await this.invokeMiddleware(context);
        }
        const route = this.findRoute(request);
        const args = await this.parseParams(request, route);
        ...
        // The second middleware chain for post-invocation processing
        context.bind('invocation.result').to(result);
        await this.invokeMiddlewareAfterInvoke(context);
        this.send(response, result);
      } catch (error) {
        this.reject(context, error);
      }
